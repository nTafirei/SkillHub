...
import com.marotech.skillhub.api.RecordingDTO;
...
public class RecordController extends BaseController {
    ...
    @ApiOperation(value = "Saves a article in the system", notes = "Saves a article in the system")
    @ApiResponses(value = {
...
/*            // 2. Save video file
            ...
            Recording article = new Recording();
            article.setName(dto.getName());
            article.setMobileNumber(dto.getMobileNumber());
            article.setDeviceLocation(dto.getDeviceLocation());
            repositoryService.save(article);

            dto.setId(article.getId());

            //Decode and save thumbnail (320x240)
            byte[] imageBytes = Base64.getDecoder().decode(imageDataBase64);
            Path thumbPath = Paths.get(uploadDir + "/" + article.getId() + "_" + article.getName() + ".jpg");
            Files.write(thumbPath, imageBytes);
            ServiceResponse response = new ServiceResponse();
            response.setResponseType(ResponseType.RECORDINGS);
            log.info("Recording uploaded: {} ({} bytes)", article.getId(), videoFile.getSize());

            try (InputStream videoStream = videoFile.getInputStream()) {
                // Save video to disk
                Path targetPath = Paths.get(uploadDir + "/" + article.getId() + "_" + article.getName());
            ...
            createActivityLog(metadataJson
                    + " saved article using web service");
            ...
            response.setCode(HttpCode.BAD_REQUEST);
            response.setResponseType(ResponseType.RECORDINGS);
            response.setMessage("An error occurred saving the article. Please contact customer service team");
            LOG.error("Error saving article for " + metadataJson, ex);
            createActivityLog(metadataJson
                    + " failed to save article using web service");
