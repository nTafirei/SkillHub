...
@UrlBinding("/web/search-by-worker")
@RequiresOneRoleOf({"Administrator", "System Administrator", "User"})
public class SearchByWorkerActionBean extends UserBaseActionBean {
    ...
    public Resolution search() throws Exception {
        ...
        if (StringUtils.isNoneBlank(firstName) || StringUtils.isNoneBlank(lastName)) {
            publications = repositoryService.findPublicationByName(firstName, lastName);
        } else {
